		...
		for (const recipient of params.recipients) {
			...
			if (index >= 0) {
				...
				if (this.recipients[index].amount < BigInt(0))
					throw new Error(`total airdrop amount can't be negative, airdrop amount for address ${this.recipients[index].address.toString('hex')} is ${this.recipients[index].amount.toString()}`);
