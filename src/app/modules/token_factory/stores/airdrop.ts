...
export class AirdropStore extends BaseStoreWithInstance<AirdropStoreData> {
	...
	public async getMutableEmptyAirdrop(ctx: MutableFactoryContext): Promise<StoreInstance<Airdrop>> {
		...
		const airdrop = new Airdrop(this.stores, this.events, this.genesisConfig!, this.factoryConfig!, this.moduleName, this.default, Buffer.alloc(0), Buffer.alloc(0));
	...
	public async getMutableAirdrop(ctx: MutableFactoryContext, tokenId: Buffer, providerAddress: Buffer): Promise<StoreInstance<Airdrop>> {
		...
		const airdrop = new Airdrop(this.stores, this.events, this.genesisConfig!, this.factoryConfig!, this.moduleName, airdropData, tokenId, providerAddress);
