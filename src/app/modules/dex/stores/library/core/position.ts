...
export async function get(
	...
	return positionInfoStore.getOrDefault(
		context,
		positionInfoStore.getKey(poolAddress, positionKey(owner, tickLower, tickUpper)),
	);
}
...
export function update(
	...
	feeGrowthInside0X128: Uint256String,
	feeGrowthInside1X128: Uint256String,
): void {
	const _self = self;
	...
	const tokensOwed0: Uint128 = Uint128.from(0).add(
		FullMath.mulDiv(
			Uint256.from(feeGrowthInside0X128).sub(_self.feeGrowthInside0LastX128).toString(),
			_self.liquidity,
			FixedPoint128.Q128,
		),
	);
	const tokensOwed1: Uint128 = Uint128.from(0).add(
		FullMath.mulDiv(
			Uint256.from(feeGrowthInside1X128).sub(_self.feeGrowthInside1LastX128).toString(),
			_self.liquidity,
			FixedPoint128.Q128,
		),
	);
