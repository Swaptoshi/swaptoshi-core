...
type Observable = {
	...
	): [tickCumulatives: Int56String[], secondsPerLiquidityCumulativeX128s: Uint160String[]];
};
...
export function consult(
...
): [arithmeticMeanTick: Int24String, harmonicMeanLiquidity: Uint128String] {
	...
	const secondsPerLiquidityCumulativesDelta = Uint160.from(
		secondsPerLiquidityCumulativeX128s[1],
	).sub(secondsPerLiquidityCumulativeX128s[0]);
	...
	const harmonicMeanLiquidity = Uint128.from(0).add(
		secondsAgoX160.div(Uint192.from(secondsPerLiquidityCumulativesDelta).shl(32)),
	);

	return [arithmeticMeanTick.toString(), harmonicMeanLiquidity.toString()];
}

export function getQuoteAtTick(
	tick: Int24String,
	baseAmount: Uint128String,
	baseToken: Buffer,
	quoteToken: Buffer,
): Uint256String {
...
export function getOldestObservationSecondsAgo(
	pool: ObservablePool,
	_timestamp: string,
): Uint32String {
	...
	const { blockTimestamp, initialized } =
		pool.observations[Uint16.from(observationIndex).add(1).mod(observationCardinality).toNumber()];
...
export function getBlockStartingTickAndLiquidity(
	pool: ObservablePool,
	timestamp: string,
): [Int24String, Uint128String] {
	...
	const {
		blockTimestamp: observationTimestamp,
		tickCumulative,
		secondsPerLiquidityCumulativeX128,
	} = pool.observations[Uint16.from(observationIndex).toNumber()];
	if (!Uint32.from(observationTimestamp).eq(timestamp)) {
		return [tick, pool.liquidity];
	}

	const prevIndex = Uint256.from(
		Uint256.from(observationIndex).add(observationCardinality).sub(1),
	).mod(observationCardinality);
	const {
	...
	const liquidity = Uint128.from(0)
		.add(
			Uint192.from(delta)
				.mul(Uint160.MAX)
				.div(
					Uint192.from(secondsPerLiquidityCumulativeX128)
						.sub(prevSecondsPerLiquidityCumulativeX128)
						.shl(32),
				),
		)
		.toString();
...
export function getWeightedArithmeticMeanTick(weightedTickData: WeightedTickData[]): Int24String {
	...
	for (let i: Uint256 = Uint256.from(0); i.lt(weightedTickData.length); i = i.add(1)) {
		numerator = numerator.add(
			Int256.from(weightedTickData[i.toNumber()].tick).mul(weightedTickData[i.toNumber()].weight),
		);
	...
	if (numerator.lt(0) && !numerator.mod(Int256.from(0).add(denominator)).eq(0))
