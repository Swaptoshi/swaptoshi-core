...
import { Uint32String, Int56String, Uint160String } from '../../../../../../../src/app/modules/dex/stores/library/int';
...
export class MockObservable {
	public constructor(secondsAgos: Uint32String[], tickCumulatives: Int56String[], secondsPerLiquidityCumulativeX128s: Uint160String[]) {
		if (!(secondsAgos.length === 2 && tickCumulatives.length === 2 && secondsPerLiquidityCumulativeX128s.length === 2)) throw new Error('Invalid test case size');
	...
	public observe(secondsAgos: Uint32String[]): [string[], string[]] {
		if (!(secondsAgos[0] === this.observation0.secondsAgo && secondsAgos[1] === this.observation1.secondsAgo)) throw new Error('Invalid test case');
