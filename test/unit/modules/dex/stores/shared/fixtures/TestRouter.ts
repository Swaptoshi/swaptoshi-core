...
export class TestRouter {
	...
	public async swapForExact0Multi(
		...
		await poolOutput.swap(
			recipient,
			false,
			Int256.from(amount0Out).mul(-1).toString(),
	...
	public async swapForExact1Multi(
		...
		await poolOutput.swap(
			recipient,
			true,
			Int256.from(amount1Out).mul(-1).toString(),
	...
	private async _swapCallback(
		...
		const {
			...
			token1: pool0Token1,
			fee: pool0Fee,
		} = PoolAddress.decodePoolAddress(Buffer.from(pool0, 'hex'));
		const pool0Inst = await poolStore.getMutablePool(
			this.context,
			pool0Token0,
			pool0Token1,
			pool0Fee,
		...
		const {
			...
			token1: pool1Token1,
			fee: pool1Fee,
		} = PoolAddress.decodePoolAddress(Buffer.from(pool1, 'hex'));
		const pool1Inst = await poolStore.getMutablePool(
			this.context,
			pool1Token0,
			pool1Token1,
			pool1Fee,
		...
		const tokenToBePaid: Buffer = Int256.from(amount0Delta).gt(0)
			? pool1Inst.token0
			: pool1Inst.token1;
