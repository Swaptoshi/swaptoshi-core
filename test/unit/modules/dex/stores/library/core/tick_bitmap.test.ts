...
async function isInitialized(tick: Int24String): Promise<boolean> {
	const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, tick, '1', true);
...
describe('TickBitmap', () => {
	...
	describe('#isInitialized', () => {
		...
		it('is not changed by another flip to a different tick on another word', async () => {
			await tickBitmap.flipTick(tickBitmapStore, methodContext, POOL_ADDRESS, (1 + 256).toString(), '1');
	...
	describe('#nextInitializedTickWithinOneWord', () => {
		...
		describe('lte = false', () => {
			it('returns tick to right if at initialized tick', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '78', '1', false);
				expect(next).toBe('84');
				expect(initialized).toBe(true);
			});
			it('returns tick to right if at initialized tick - 2', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '-55', '1', false);
				expect(next).toBe('-4');
				expect(initialized).toBe(true);
			});

			it('returns the tick directly to the right', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '77', '1', false);
				expect(next).toBe('78');
				expect(initialized).toBe(true);
			});
			it('returns the tick directly to the right - 2', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '-56', '1', false);
				expect(next).toBe('-55');
				expect(initialized).toBe(true);
			});

			it('returns the next words initialized tick if on the right boundary', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '255', '1', false);
				expect(next).toBe('511');
				expect(initialized).toBe(false);
			});
			it('returns the next words initialized tick if on the right boundary - 2', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '-257', '1', false);
			...
			it('returns the next initialized tick from the next word', async () => {
				...
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '328', '1', false);
				expect(next).toBe('340');
				expect(initialized).toBe(true);
			});
			it('does not exceed boundary', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '508', '1', false);
				expect(next).toBe('511');
				expect(initialized).toBe(false);
			});
			...
			it('skips half word', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '383', '1', false);
				expect(next).toBe('511');
				expect(initialized).toBe(false);
			});
		});
		...
		describe('lte = true', () => {
			it('returns same tick if initialized', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '78', '1', true);

				expect(next).toBe('78');
				expect(initialized).toBe(true);
			});
			it('returns tick directly to the left of input tick if not initialized', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '79', '1', true);

				expect(next).toBe('78');
				expect(initialized).toBe(true);
			});
			it('will not exceed the word boundary', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '258', '1', true);

				expect(next).toBe('256');
				expect(initialized).toBe(false);
			});
			it('at the word boundary', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '256', '1', true);

				expect(next).toBe('256');
				expect(initialized).toBe(false);
			});
			it('word boundary less 1 (next initialized tick in next word)', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '72', '1', true);

				expect(next).toBe('70');
				expect(initialized).toBe(true);
			});
			it('word boundary', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '-257', '1', true);

				expect(next).toBe('-512');
				expect(initialized).toBe(false);
			});
			it('entire empty word', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '1023', '1', true);

				expect(next).toBe('768');
				expect(initialized).toBe(false);
			});
			it('halfway through empty word', async () => {
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '900', '1', true);
			...
			it('boundary is initialized', async () => {
				...
				const [next, initialized] = await tickBitmap.nextInitializedTickWithinOneWord(tickBitmapStore, methodContext, POOL_ADDRESS, '456', '1', true);
