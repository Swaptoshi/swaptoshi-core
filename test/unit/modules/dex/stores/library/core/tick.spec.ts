...
describe('Tick', () => {
	...
	async function getTick(tickNumber: string) {
		return tickInfoStore.getOrDefault(
			methodContext,
			tickInfoStore.getKey(POOL_ADDRESS, tickNumber),
		);
	}
	...
	describe('#tickSpacingToMaxLiquidityPerTick', () => {
		it('returns the correct value for low fee', () => {
			const maxLiquidityPerTick = tickTest.tickSpacingToMaxLiquidityPerTick(
		...
		it('reverts on overflow liquidity gross', async () => {
			const func = async () => {
				await tickTest.update(
					...
					'0',
					'0',
					MaxUint128.div(2).sub(1).toString(),
					'0',
					'0',
					'0',
					'0',
