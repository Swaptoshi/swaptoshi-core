...
import { EventQueue } from 'klayr-framework/dist-node/state_machine';
import {
	MethodContext,
	createMethodContext,
} from 'klayr-framework/dist-node/state_machine/method_context';
import { PrefixedStateReadWriter } from 'klayr-framework/dist-node/state_machine/prefixed_state_read_writer';
import { InMemoryPrefixedStateDB } from 'klayr-framework/dist-node/testing/in_memory_prefixed_state';
import {
...
import { NFTStore, nftStoreSchema } from '../../../../src/app/modules/nft/stores/nft';
import { UserStore } from '../../../../src/app/modules/nft/stores/user';
import { DestroyEvent, DestroyEventData } from '../../../../src/app/modules/nft/events/destroy';
import { SupportedNFTsStore } from '../../../../src/app/modules/nft/stores/supported_nfts';
import { CreateEvent } from '../../../../src/app/modules/nft/events/create';
import { LockEvent, LockEventData } from '../../../../src/app/modules/nft/events/lock';
import { InternalMethod } from '../../../../src/app/modules/nft/internal_method';
import { TransferEvent, TransferEventData } from '../../../../src/app/modules/nft/events/transfer';
import {
	TransferCrossChainEvent,
	TransferCrossChainEventData,
} from '../../../../src/app/modules/nft/events/transfer_cross_chain';
import { AllNFTsSupportedEvent } from '../../../../src/app/modules/nft/events/all_nfts_supported';
import { AllNFTsSupportRemovedEvent } from '../../../../src/app/modules/nft/events/all_nfts_support_removed';
import {
	AllNFTsFromChainSupportedEvent,
	AllNFTsFromChainSupportedEventData,
} from '../../../../src/app/modules/nft/events/all_nfts_from_chain_suported';
import {
	AllNFTsFromCollectionSupportRemovedEvent,
	AllNFTsFromCollectionSupportRemovedEventData,
} from '../../../../src/app/modules/nft/events/all_nfts_from_collection_support_removed';
import {
	AllNFTsFromCollectionSupportedEvent,
	AllNFTsFromCollectionSupportedEventData,
} from '../../../../src/app/modules/nft/events/all_nfts_from_collection_suppported';
import {
	AllNFTsFromChainSupportRemovedEvent,
	AllNFTsFromChainSupportRemovedEventData,
} from '../../../../src/app/modules/nft/events/all_nfts_from_chain_support_removed';
import { RecoverEvent, RecoverEventData } from '../../../../src/app/modules/nft/events/recover';
import {
	SetAttributesEvent,
	SetAttributesEventData,
} from '../../../../src/app/modules/nft/events/set_attributes';
import { EscrowStore } from '../../../../src/app/modules/nft/stores/escrow';
import { UnlockEvent, UnlockEventData } from '../../../../src/app/modules/nft/events/unlock';
import { NFTModule, NFTMethod } from '../../../../src/app/modules/nft';
...
describe('NFTMethod', () => {
	...
	let existingNFT: { nftID: Buffer; owner: Buffer };
	let existingNativeNFT: { nftID: Buffer; owner: Buffer };
	let lockedExistingNFT: { nftID: Buffer; owner: Buffer; lockingModule: string };
	let escrowedNFT: { nftID: Buffer; owner: Buffer };
	...
	describe('isNFTEscrowed', () => {
		it('should return true if nft owner is a chain', () => {
			expect(method.isNFTEscrowed({ ...escrowedNFT, attributesArray: [] })).toBeTruthy();
		});

		it('should return false if nft owner is not a chain', () => {
			expect(method.isNFTEscrowed({ ...existingNFT, attributesArray: [] })).toBeFalsy();
		});
	});
	...
	describe('isNFTLocked', () => {
		it('should return true if nft is locked', () => {
			expect(method.isNFTLocked({ ...lockedExistingNFT, attributesArray: [] })).toBeTruthy();
		});

		it('should return false if nft does not have locking module property', () => {
			expect(method.isNFTLocked({ ...existingNFT, attributesArray: [] })).toBeFalsy();
		});
	...
	describe('destroy', () => {
		...
		it('should delete NFTStore and UserStore entry and emit Destroy event', async () => {
			...
			await expect(nftStore.has(methodContext, existingNFT.nftID)).resolves.toBeFalsy();
			await expect(
	...
	describe('transferCrossChain', () => {
		...
		it('should throw and emit error transfer cross chain event if balance is less than message fee', async () => {
			...
			interopMethod.getMessageFeeTokenID.mockResolvedValue(messageFeeTokenID);
	...
	describe('supportAllNFTs', () => {
		it('should remove all existing entries, add ALL_SUPPORTED_NFTS_KEY entry and log AllNFTsSupportedEvent', async () => {
			...
			await expect(supportedNFTsStore.has(methodContext, chainID)).resolves.toBeFalsy();
	...
	describe('removeSupportAllNFTs', () => {
		it('should remove all existing entries and log AllNFTsSupportRemovedEvent', async () => {
			...
			await expect(supportedNFTsStore.has(methodContext, chainID)).resolves.toBeTruthy();
